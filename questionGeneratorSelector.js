...
function getChoiceRow() {
  ...
  let choiceListNew = `<div class="row rowChoice align-items-center">
    <div  class="col-2 numerical-input m-1 p-1" >
      <!-- <input type="number" class = "questionAnswerSort missing border-primary"> -->
      <input type="number" class = "questionAnswerSort border-primary w-100">
    </div>
    <div class="questionAnswerDescription missing col-4 p-2" contenteditable="true"></div>
    <div class="questionExplanationRequireYesNo col-4 p-3 border border-warning d-flex flex-column align-items-center" >
      <div class="d-flex align-items-center mb-2">
        <input type="checkbox" class="checkboxExplain mr-2"> Require explanation      
        <input type="checkbox" class="checkboxNoChange hidealways">
      </div>
      <div class="d-flex w-100 justify-content-between">
        <button type="button" class="btn btn-primary btn-sm copyBtn flex-grow-1 mr-1">Insert Copy</button>
        <button type="button" class="btn btn-danger btn-sm deleteBtn deleteBtn2 flex-grow-1 ml-1">Delete</button>
      </div>
    </div>
    <!-- <div class="questionAnswerNotesDescription col-2" contenteditable="true"></div> -->
...
// this could replace other location if it is parsed, "HOPE TO SEE"
  let temp =  ` <div class="text-primary col-2 font-weight-bold">
  Position Number<br>&nbsp a) unique and numeric; <br>&nbsp b) negative and/or decimal point
</div>
<div class="questionAnswerTitle col-4 text-primary font-weight-bold">Formattable Choice Description </div>
<div class="questionAnswerTitle col-4 text-warning font-weight-bold">Answer per Question Setup</div>

  <div class="questionAnswerTitle col-2 text-danger trueRuleDiv font-weight-bold" style = "text-align:center"> Check to Display<br>    
...
function getQuestionaireGroupRowChoice () {
  ...
  let temp =  `
  <div class="row rowChoice m-0 align-items-center" style="display: flex; align-items: center;">
      <div class="col-1"></div>
      <div class="col-1"></div>
      <div class="col-5"><span class = "questionRuleDescription"><button class="btn btn-outline-primary QuestionnaireRule questionnaireSelectRule" type="submit" style="width: 100%">Select Display Rule or Collection </button></span></div>
      <div  class="col-3 questionnairecloud" > </div>
      <div  class="col-2" style="display: flex; flex-direction: column; gap: 0.5rem; justify-content: center;"><button type="button" class="btn btn-danger deleteBtn" style="width: 100%">Delete</button><button type="button" class="btn btn-primary copyBtn" style="width: 100%">Insert</button></div>
    </div>  
  `;
  return  temp
}
...
function saveTrueRule() {
  console.log("function saveTrueRule() {");
    ...
    // Validation: Check only ruleAuthor and ruleID (not questionAuthor/questionID as they might be set from loaded rule)
    let ruleAuthorElement = document.getElementById("ruleAuthor");
    let ruleIDElement = document.getElementById("ruleID");
    
    if (!ruleAuthorElement || !ruleAuthorElement.innerText || ruleAuthorElement.innerText.trim() === "" ||
        !ruleIDElement || !ruleIDElement.innerText || ruleIDElement.innerText.trim() === "") {
      console.warn("Missing rule author or ID");
      showSnackbar("⚠️ Please fill in Rule Author and Rule ID");
      if (ruleAuthorElement && (!ruleAuthorElement.innerText || ruleAuthorElement.innerText.trim() === "")) {
        ruleAuthorElement.classList.add("missing");
      }
      if (ruleIDElement && (!ruleIDElement.innerText || ruleIDElement.innerText.trim() === "")) {
        ruleIDElement.classList.add("missing");
      }
    ...
    for (let index = 0; index < objectKeyList.length; index++) {
      ...
      let element = document.getElementById(objectKey);
      objectValue = element ? element.innerText.trim() : "";
      saveObject[objectKey] = objectValue
      //getInnerText (saveObject, objectKey, objectValue)
    }
    
    // Additional check: if questionAuthor or questionID is missing, show helpful message
    if (!saveObject.questionAuthor || !saveObject.questionID) {
      console.warn("Question not selected for this rule");
      showSnackbar("⚠️ Please click 'Select / Change Question for Rule' button to choose a question first");
      return;
    }
    ...
    // Validation: Check if at least one answer is selected
    if (trueAnswerList.length === 0) {
      console.warn("No answers selected for the rule");
      showSnackbar("⚠️ Please select at least one answer for this display rule");
      return; // Stop the save process
    }
    ...
    showSnackbar("✅ Display rule saved successfully!");
    console.log("Display rule saved:", saveObject);
    db.close()
  }
}

function saveRuleOnly() {
  console.log("function saveRuleOnly() - Simple save without question validation");
  console.log("=== SAVE RULE ONLY - START ===");
  
  // Get rule fields
  const ruleAuthor = document.getElementById("ruleAuthor");
  const ruleID = document.getElementById("ruleID");
  const ruleContent = document.getElementById("ruleContent");
  
  console.log("Rule fields:", {
    ruleAuthor: ruleAuthor ? ruleAuthor.innerText.trim() : "NOT FOUND",
    ruleID: ruleID ? ruleID.innerText.trim() : "NOT FOUND",
    ruleContent: ruleContent ? ruleContent.innerText.trim() : "NOT FOUND"
  });
  
  // Basic validation - only check rule fields
  if (!ruleAuthor || !ruleAuthor.innerText.trim()) {
    console.warn("Validation failed: Rule Author missing");
    showSnackbar("⚠️ Please enter Rule Author");
    if (ruleAuthor) ruleAuthor.classList.add("missing");
    return;
  }
  
  if (!ruleID || !ruleID.innerText.trim()) {
    console.warn("Validation failed: Rule ID missing");
    showSnackbar("⚠️ Please enter Rule ID");
    if (ruleID) ruleID.classList.add("missing");
    return;
  }
  
  if (!ruleContent || !ruleContent.innerText.trim()) {
    console.warn("Validation failed: Rule Content missing");
    showSnackbar("⚠️ Please enter Rule Description");
    if (ruleContent) ruleContent.classList.add("missing");
    return;
  }
  
  console.log("✓ Validation passed");
  
  // Build save object with only rule data
  const saveObject = {
    ruleAuthor: ruleAuthor.innerText.trim(),
    ruleID: ruleID.innerText.trim(),
    ruleContent: ruleContent.innerHTML,
    ruleContentInnerText: ruleContent.innerText.trim(),
    keyPathValue: ["trueruleStore", 1, ruleAuthor.innerText.trim(), 1, ruleID.innerText.trim()],
    versionDateSince1969: Date.now(),
    localTimeString: new Date().toString(),
    trueAnswerList: [],
    trueAnswerDescriptionList: [],
    questionAuthor: "",
    questionID: "",
    savedWithSimpleSave: true
  };
  
  console.log("Save object created:", saveObject);
  
  // Add question data if available (optional)
  const questionAuthor = document.getElementById("questionAuthor");
  const questionID = document.getElementById("questionID");
  
  if (questionAuthor && questionAuthor.innerText.trim()) {
    saveObject.questionAuthor = questionAuthor.innerText.trim();
    console.log("Added questionAuthor:", saveObject.questionAuthor);
  }
  if (questionID && questionID.innerText.trim()) {
    saveObject.questionID = questionID.innerText.trim();
    console.log("Added questionID:", saveObject.questionID);
    if (saveObject.questionAuthor) {
      saveObject.questionSource = ["questionStore", 1, saveObject.questionAuthor, 1, saveObject.questionID];
      console.log("Added questionSource:", saveObject.questionSource);
    }
  }
  
  // Collect selected answers if any
  const truefalseRuleList = document.getElementsByClassName("trueRule");
  const questionAnswerSortList = document.getElementsByClassName("questionAnswerSort");
  const questionAnswerDescriptionList = document.getElementsByClassName("questionAnswerDescription");
  
  console.log("Found answer checkboxes:", truefalseRuleList.length);
  
  if (truefalseRuleList.length > 0) {
    const trueAnswerList = [];
    const trueAnswerDescriptionList = [];
    
    for (let index = 0; index < truefalseRuleList.length; index++) {
      const element = truefalseRuleList[index];
      if (element.checked) {
        const element2 = questionAnswerSortList[index];
        const element3 = questionAnswerDescriptionList[index];
        if (element2 && element3) {
          trueAnswerList.push(element2.value);
          trueAnswerDescriptionList.push(element3.innerHTML);
        }
      }
    }
    ...
    saveObject.trueAnswerList = trueAnswerList;
    saveObject.trueAnswerDescriptionList = trueAnswerDescriptionList;
    console.log("Added answers:", trueAnswerList.length, "checked");
  }
  ...
  request.onerror = function(event) {
    ...
    showSnackbar("❌ Error opening database");
  };
  ...
  request.onsuccess = function() {
    ...
    try {
      ...
      console.log("Saving to ruleStore with keyPath:", saveObject.keyPathValue);
      const putRequest = store.put(saveObject);
      
      putRequest.onsuccess = function() {
        console.log("✓ Rule saved to ruleStore successfully!");
        showSnackbar("✅ Rule saved successfully!");
        
        // Remove missing class from fields
        if (ruleAuthor) ruleAuthor.classList.remove("missing");
        if (ruleID) ruleID.classList.remove("missing");
        if (ruleContent) ruleContent.classList.remove("missing");
        
        console.log("=== SAVE RULE ONLY - SUCCESS ===");
      };
      
      putRequest.onerror = function(event) {
        console.error("❌ Error saving rule to ruleStore:", event.target.error);
        showSnackbar("❌ Error saving rule: " + event.target.error.message);
      };
...
async function getAllFromFireStore () {
  ...
  showSnackbar("Retrieving data from server...");
  try {
  ...
  promises = [ promise0, promise1, promise2 ] ;
    ...
    showSnackbar("Data retrieval complete!");
    console.log("Data retrieval complete");
  } catch (error) {
    console.error("Error retrieving data:", error);
    showSnackbar("Error retrieving data. Check console.");
  }
}
...
function modalJoeFilter() {
  ...
  for (i = 0; i < li; i++) {
      ...
      if (a && (a.textContent || a.innerText)) {
        txtValue = a.textContent || a.innerText;
      } else {
        txtValue = "not a valid entry 2021-02-10";
      }
...
function upDateModal(allSavedItems , selectObject, workingQuestionnaire) {
  ...
  for (let index = 0; index < allSavedItems.length; index++) {
    ...
    let content = `<div class="row align-items-center m-0 w-100">
      <div class="col-9">
        <div class="mb-1">
          <span class="text-muted small">Author:</span> <span class="modal-author font-weight-bold text-primary">${element.keyPathValue[2]}</span>
          <span class="text-muted small ml-2">ID:</span> <span class="modal-ID font-weight-bold text-primary">${element.keyPathValue[4]}</span>
        </div>`
    if (element.keyPathValue[0] === "questionStore") {
      content = content +`<div class="mb-1"><span class="text-muted small">Formattable Question Wording:</span>` ;
      //row.className = "questionWrapper" 
      row.classList.add("questionWrapper")
    } else if(element.keyPathValue[0] === "trueruleStore") {
      content = content +`<div class="mb-1"><span class="text-muted small">Display Rule:</span>` ;
      //row.className = "ruleWrapper" 
      row.classList.add("ruleWrapper")
    } else if(element.keyPathValue[0] === "groupruleStore") {
      content = content +`<div class="mb-1"><span class="text-muted small">Display Rule Collection:</span>` ;
      //row.className = "groupruleWrapper"
      row.classList.add("groupruleWrapper") 
    }  else if(element.keyPathValue[0] === "questionnaireStore") {
      content = content +`<div class="mb-1"><span class="text-muted small">Questionnaire Title:</span>` ;
    ...
    if(workingQuestionnaire === "buildSelectQuestionnaire") {
       onclickAction = `buildSaveModalAnswer('${t}')`;
    } else if(workingQuestionnaire === "useQuestionnaire") {
       onclickAction = `buildSaveModalAnswerUse('${t}')`;
    } else {
       onclickAction = `saveModalAnswer('${t}')`;
    }
    content = content + ` <span class="modal-word font-weight-bold">${elementByStore.content}</span></div></div>` +
      `<div class="col-3 text-right"><button type="button" class="btn btn-accent btn-sm w-100" onclick="${onclickAction}">Retrieve</button></div></div>` ;
    row.innerHTML = content ;
    //place.appendChild(row) ;
    frag.appendChild(row) ;
    t = 0 ;
...
function storeGroupruleInDOM (questionnaireOutline){
  ...
  // Handle new rule logic types (2026-01-29) - check if the new dropdown exists
  const ruleLogicTypeElement = document.getElementById("ruleLogicType");
  if (ruleLogicTypeElement) {
    // New logic: use ruleLogicType dropdown
    if (questionnaireOutline.ruleLogicType) {
      ruleLogicTypeElement.value = questionnaireOutline.ruleLogicType;
      ...
      if (typeof updateGroupRuleLogicDisplay === 'function') {
      ...
      ruleLogicTypeElement.value = questionnaireOutline.all0Any1True ? "any" : "all";
      if (typeof updateGroupRuleLogicDisplay === 'function') {
        updateGroupRuleLogicDisplay();
      }
    } else {
    ...
    // Old logic: use all0Any1True checkbox (for backward compatibility)
    const all0Any1Element = document.getElementById("all0Any1True");
    if (all0Any1Element) {
      all0Any1Element.checked = questionnaireOutline.all0Any1True;
      if (typeof all0Any1 === 'function') {
        all0Any1();
      }
    }
  }
  
  if(questionnaireOutline.groupruleContentInnerText && questionnaireOutline.groupruleContentInnerText.trim() !== "") {
    temp0 = document.getElementById("groupruleContent") ;
    temp0.innerHTML = questionnaireOutline.groupruleContent ;
    temp0.classList.remove("missing");
  } else if (questionnaireOutline.groupruleContent) {
    // Fallback if only groupruleContent exists
    temp0 = document.getElementById("groupruleContent") ;
    temp0.innerHTML = questionnaireOutline.groupruleContent ;
    temp0.classList.remove("missing");
  }
...
function retreiveGrouprule (keyPathValue) {
  ...
  console.log("Retrieving group rule with keyPathValue:", keyPathValue);
  let selectObject = {} ;
  ...
  databaseOpenRequest.onsuccess = () => {
    ...
    rowObject.onsuccess = () => {
      let indexedDBRecordRetreived = rowObject.result ;
      console.log("Retrieved group rule data:", indexedDBRecordRetreived);
      
      if (!indexedDBRecordRetreived) {
        console.error("No data found for keyPathValue:", keyPathValue);
        const errorMsg = `Error: Could not find rule data in database.\n\nKeyPath: ${JSON.stringify(keyPathValue)}\n\nPossible solutions:\n1. The rule hasn't been created yet\n2. Open ManageRules.html and click "Create Sample Data"\n3. Create a new rule with Author: ${keyPathValue[2]}, ID: ${keyPathValue[4]}`;
        alert(errorMsg);
        console.log("💡 Tip: Open ManageRules.html to create sample data or verify existing rules");
        return;
      }
    ...
    rowObject.onerror = (event) => {
      console.error("Error retrieving group rule:", event);
      alert("Error retrieving rule from database");
    };
...
function emptyQuestionnaire() {
  ...
  console.log("DEBUG: emptyQuestionnaire called");
  menuChoicesToggle(1);
  hideEmptyAllWrappers() ; 
  ...
  <div class="row">
    ...
    <div class="row">
      ...
      <div style="margin-bottom: 15px;">
        ...
        <input id="saveRuleOnlyButton" type="Button" class="btn btn-success" onclick="saveRuleOnly()" value="💾 Save Rule Only" style="margin-left: 10px; font-weight: bold;">
        <small style="display: block; margin-top: 5px; color: #666;">Use 'Save Rule Only' to save rule without question validation</small>
      </div>
...
function setupAllWrapperEvents0() {
  ...
  console.log(`document.addEventListener("DOMContentLoaded", function() - this needs to change so that value is loged in author`);
