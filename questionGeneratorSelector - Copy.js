...
function processSourceRuleLists(completeList, count , allItems , newAnswer, priorTrueFalse) {
  // just to have something here, will be needed to process when ready
...
function buildsSourceRuleLists (changedAnswerKey, newAnswer, priorTrueFalse) {
  // this determines which items are displayed  based on source
  // list to dermine which lists are affected by changes to answers, done dynamically based on what is in the database. 
  // we have the source cell, (changedAnswerKey)
  //let completeList = {} ;

    /*   ***

  let completeList = new Map(); //Empty Map
  let count = 0;
  let allItems = [];
  allItems.push(changedAnswerKey) ;
  getSourceRuleLists (completeList, count , allItems , newAnswer, priorTrueFalse) ;
    */
}
    ...
    tx.oncomplete = () => {
      ...
      if(allSavedItems.result !== undefined) {
        if( toConfirm === false) {
          ...
          Title: ${allSavedItems.result.ruleContentInnerText}
...
function getEmptyGroupHeader() {
  ...
  <div class="container" >
    ...
    <div class="row">
      ...
      <!--<div id="groupruleAuthor" class="center questionQuestionContent required col-2 m-1"></div>-->
    </div>
    <div class="row">
...
function notBlankId () {
  let Id = document.getElementById("questionnaireID").innerText ;
  if(!Id) {
    //return  "noDocId"
    return  null
  }
...
function notBlankAuthor () {
  ...
  let author = document.getElementById("questionnaireAuthor").innerText;
  if(!author) {
    //return  "noAuthor"
    return  null
  }
...
function initializeFireStore () {
  // to be used later to remove need to repeated code.
}
...
function parseNestedArrays (saveObject) {
  //console.log("function parseNestedArrays (saveObject) 1") ;
...
function getNewPendingObjectToSave () {
  return newPendingObjectToSave
}
...
function saveGrouprule() {
  ...
  grouprule.questionSource = grouprule.keyPathValue;
  ...
  databaseOpenRequest.onsuccess = () => {
    ...
    tx.oncomplete = () => {
      ...
      console.log("*** ***saved to indexeddb, not to firestore *** ***")
    }
  }
}
...
function setRuleKeyPathValueInCombinationDetail (saveObject) {
  console.log("function setRuleKeyPathValueInCombinationDetail  - first item Object");
  console.log("saveObject");
  console.log(saveObject);
...
async function saveToDatabase(saveObject, questionnaireRowChoice) { 
  ...
  /*
  saves both questionnaires and questions

  "questionnaireRowChoice" is undefined if saving a question, if saving a questionnaire it is is defined, it may be possible to elimiate this variable because of details in the saveObject, for now both are needed because both are used.

  saves to both databases, indexedDB and FireStore

  save Object is record to be saved to database after modification for missing information. 
...
function getGroupruleAnswerRows(questionnaireOutline) {
  ...
  for (let index = 0; index < docLength; index++) {
    ...
    newRow.getElementsByClassName("questionnairecloud")[0].innerHTML = "";
...
function doModalChoice(offset) {
  let choices = [" this would be zero"," this would be one"] ;
  alert(`Hopefully this is zero:   ${choices[offset]}`)
}
...
function updateScreenQuestionStorePart2(saveObject) {
  console.log("updateScreenQuestionStorePart2(saveObject)");
...
function doSameAllAuthorStartButtons (buttonTitle) { // cache buttonClicked  information to be used to later add correct data input fields and apply correct formatting
...
function saveNewScreenButtonClicked(event,buttonTitle) { // setup screen for new question
  console.log("function saveNewQuestionButtonClicked(buttonTitle)") ;
...
function checkMultiLevelItem( keyPathValue , ruleTrueFalse, parent, preSave , parent2 ) {
  console.log(`*** 
  checkMultiLevelItem( preSave , ruleTrueFalse , keyPathValue) 
  ***`) ;
  console.log("keyPathValue") ;
  console.log(keyPathValue) ;
...
function saveDataAnswer (replacement) {
  console.log("saveDataAnswer (replacement)") ;
  let open = indexedDB.open ("authorExcuTrust");
  open.onsuccess = function () {
    db = open.result;
...
function saveRuleIfConclusionDifferentAndStartGroupRules () {
...
function getWaitForList(waitForList) {
...
async function makeConclusionsForTheseFacts0 (db , facts) { // db is only a placeholder
  console.log("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&      async function makeConclusionsForTheseFacts0 (db , facts)")
 ...
 async function makeConclusionsForTheseFacts (db ,facts) {
  console.log ("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++        function makeConclusionsForTheseFacts (db ,facts) {")
...
function checkHideDescrption(screen) {
  if(screen === "NewQuestion") {
...
function saveLogicalAnswerToDataBase (theAnswerElement , event) { // *** event does not appear to be relevant here  
  ...
  console.log("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^     function saveLogicalAnswerToDataBase 1 ") ;
  let logicalAnswer = {} ;
...
function toggleAnswer (event) {
  console.log("::::::::::::::::::::::::::::::::::::    4  toggleAnswer (event) ::::::::::::::::") ;
  if( 1 === 1) {
    console.log("if( 1 === 1)") ;
    toggleAnswer1(event)
  } else {
...
function getScreenChoices() {
    ...
    'newQuestion': () => newemptyQuestion(),
    'newDisplayRule': () => emptyRule31(), //alert ("newDisplayRule"),
    'newQuestionnaire':  () => emptyQuestionnaire(), // alert("newQuestionnaire"),
    'collectionOfDisplayRules':() => emptyGroupRule() //" alert("collectionOfDisplayRules")
  } 
  return hope
}
...
function clickEventsMenuChoices(event) {
  ...
  if(buttonPressed) {
    ...
    console.log(screenChoices.buttonPressed);
...
function clickEventsAllWrappers (event) {  
  console.log ("function clickEvents (event)");
  event.stopPropagation() ;
