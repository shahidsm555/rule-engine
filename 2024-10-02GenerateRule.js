...
function groupruleHTMLElement(indexedDBRecord) {
    console.log("function groupruleHTMLElement(indexedDBRecord) {");
    // remember to use 
    let wrapper, collectionT, htmlDescription;
    ...
    let theRow = `<div data-questionnairerowruletype="${indexedDBRecord.keyPathValue[0]}" data-questionnairerowruleauthor="${indexedDBRecord.keyPathValue[2]}" data-questionnairerowruletid="${indexedDBRecord.keyPathValue[4]}" class="row rowChoice m-0 ${wrqpper} modal-filter-row">
        <div col-1"=""> </div>
        <div col-1"=""> </div>
  ...
  function getIndexedDBRuleArray(finalList , myStore, sourceDB) {
    console.log("function getIndexedDBRuleArray(finalList , ruleStore, authorExcuTrust) {, actually finalList, ruleStore, authorExecTrust 2024-08-01");
  ...
  function getObjectFromIndexedDB(keyPathValue) {
    let data;
    // Open the database
  ...
  function getHtmlWording(theObject) {
    ...
    if( theObject.keyPathValue[0] === "groupruleStore") {
      ...
      tempText = `<div data-questionnairerowruletype="trueruleStore" data-questionnairerowruleauthor="${theObject.keyPathValue[2]}" data-questionnairerowruletid="${theObject.keyPathValue[4]}" class="row rowChoice m-0 ruleWrapper">
        <div col-1"=""> </div>
        <div col-1"=""> </div>
        <div class="col-6"><span class="questionRuleDescription"><button class="QuestionnaireRule questionnaireSelectRule " type="submit">Select Display Rule or Collection </button> <span><b>Display Rule</b><br></span><b>Author:&nbsp;</b><span class="questionnaireRowRuleAuthor modal-author">${theObject.keyPathValue[2]}</span><b>&nbsp; ID:&nbsp;</b><span class="questionnaireRowRuleID modal-ID">${theObject.keyPathValue[4]}</span><br><span style="background-color: rgb(255, 255, 255);">${ruleDescription}</span></span></div>
        <div class="col-3 questionnairecloud">Saved</div>
        <div class="col-1"><input type="Button" class="deleteBtn" value="Delete"><br><br><input type="Button" class="copyBtn" value="Insert"></div>
      </div>`
    }
    ...
    console.log(tempText);
    let questionAnswerChoices = document.getElementById("questionAnswerChoices");
    questionAnswerChoices.insertAdjacentHTML("beforeend",tempText);
    return tempText
  }
  ...
  async function getRowHtmlV2(grouprule) {
    ...
    request.onsuccess = async function(event) {
      ...
      for(let counter = 0; counter < theLength; counter++) {
        ...
        console.log(counter);
        let getRequest =  store.get(grouprule.parsedReducedList[counter]);
        getRequest.onsuccess = function (event){
          let theObject = event.target.result;
          grouprule.tAllRowChoices.push (getHtmlWording(theObject));
          console.log("*** grouprule 2021-10-01 18:59 ***");
          console.log(grouprule)
        }
        getRequest.onerror = async function(event) {
          console.error('Error retrieving object:',  event.target.error);
    ...
    console.log("grouprule after db.close()");
    console.log(grouprule);
    return grouprule
  }
  ...
  async function getRowHtml(element) {
    ...
    if( element[0] === "groupruleStore") {
      tempText =`<div data-questionnairerowruletype="groupruleStore" data-questionnairerowruleauthor="${element[2]}" data-questionnairerowruletid="${element[4]}" class="row rowChoice m-0 groupruleWrapper">
        <div col-1"=""> </div>
        <div col-1"=""> </div>
        <div class="col-6"><span class="questionRuleDescription"><button class="QuestionnaireRule questionnaireSelectRule " type="submit">Select Display Rule or Collection </button> <span><b>Display Rule Collection</b><br></span><b>Author:&nbsp;</b><span class="questionnaireRowRuleAuthor modal-author">${element[2]}</span><b>&nbsp; ID:&nbsp;</b><span class="questionnaireRowRuleID modal-ID">${element[4]}</span><br>${ruleDescription}</span></div>
        <div class="col-3 questionnairecloud">Saved</div>
        <div class="col-1"><input type="Button" class="deleteBtn" value="Delete"><br><br><input type="Button" class="copyBtn" value="Insert"></div>
      </div>`
    } else {
      tempText = `<div data-questionnairerowruletype="trueruleStore" data-questionnairerowruleauthor="${element[2]}" data-questionnairerowruletid="${element[4]}" class="row rowChoice m-0 ruleWrapper">
        <div col-1"=""> </div>
        <div col-1"=""> </div>
        <div class="col-6"><span class="questionRuleDescription"><button class="QuestionnaireRule questionnaireSelectRule " type="submit">Select Display Rule or Collection </button> <span><b>Display Rule</b><br></span><b>Author:&nbsp;</b><span class="questionnaireRowRuleAuthor modal-author">${element[2]}</span><b>&nbsp; ID:&nbsp;</b><span class="questionnaireRowRuleID modal-ID">${element[4]}</span><br><span style="background-color: rgb(255, 255, 255);">${ruleDescription}</span></span></div>
        <div class="col-3 questionnairecloud">Saved</div>
        <div class="col-1"><input type="Button" class="deleteBtn" value="Delete"><br><br><input type="Button" class="copyBtn" value="Insert"></div>
      </div>`
    }
...
function do2CommandsForEach(element, questionAnswerChoices){
    console.log("function do2CommandsForEach(element, questionAnswerChoices){");
    console.log(element);
    questionAnswerChoices.insertAdjacentHTML("beforeend",element);
    setTimeout(() => {
        console.log('Adding new HTML');
      }, 2000)
}
function appendRowChoices(grouprule) {
    console.log("function appendRowChoices(groupRule) {");
    //let tempArray = [];
    //grouprule.tAllRowChoices.forEach((element) => tempArray.unshift(element));
    let questionAnswerChoices = document.getElementById("questionAnswerChoices");
    //tempArray.forEach((element) => questionAnswerChoices.insertAdjacentHTML('afterend', element));
    //tempArray.forEach((element) => do2CommandsForEach(element, questionAnswerChoices))
    console.log("element 5 times? and console.log(groupruletAllRowChoices)");
    console.log(grouprule.tAllRowChoices);
    let tempArray = Array.from(grouprule.tAllRowChoices);
    console.log(" first a list then a foreachloop of grouprule.tAllRowChoices.forEach((element) => console.log(element)); ten");
    console.log(grouprule.tAllRowChoices[0]);
    console.log(grouprule.tAllRowChoices[1]);
    console.log(grouprule.tAllRowChoices[2]);
    console.log(grouprule.tAllRowChoices[3]);
    console.log(grouprule.tAllRowChoices[4]);
  ...
  //make function which removes all elments with class rowChoice except the first one
    /* function removeRowChoice() {
        console.log("function removeRowChoice() {");
        let rowChoice = document.getElementsByClassName("rowChoice");
        let rowChoiceLength = rowChoice.length;
        for (let index = 1; index < rowChoiceLength; index++) {
            const element = rowChoice[index];
            element.remove();
        }
    }
    */
// function to remove all elements with class rowChoice
function removeAllRowChoice() {
    console.log("function removeAllRowChoice() {");
    var paras = document.getElementsByClassName("rowChoice");
    while(paras[0]) {
...
/*
        let rowChoice = document.getElementsByClassName("rowChoice");
        let rowChoiceLength = rowChoice.length;
        for (let index = 0; index < rowChoiceLength; index++) {
        


    // function to replace element with class rowChoice with new element and append new elements wherearray of html text os provvided with array named TallRowChoices
    function replaceRowChoice(groupRule){
        console.log("function replaceRowChoice(groupRule){");
        let originalElements = document.getElementsByClassName("rowChoice");
  ...
  function fixAllRowChoicesV2(grouprule){
    console.log("function fixAllRowChoicesV2(grouprule){ 888 2024-10-01-1516 ***");
    removeAllRowChoice();
    grouprule.tAllRowChoices = [];
    grouprule = getRowHtmlV2(grouprule);
    return grouprule
  }
  
  function fixAllRowChoices(grouprule){
    grouprule.tAllRowChoices = [];
    grouprule.parsedReducedList.forEach((element) => grouprule.tAllRowChoices.push(getRowHtml(element)));
    return grouprule
  }
  ...
  async function saveGrouprule() {
    ...
    console.log("allArrayTemp  ***   2024-08-07   18:49 ***   *********************************************************************************************************************************************************************************************  from buildAllArraytemp - will not be needed!");
    ...
    grouprule.reducedstringIfiedFilteredList =  buildKeyPathListForStringfied() ;
    ...
    console.log("*** 2024-10-01 18:44 group rule*** ");
    console.log(grouprule);
    console.log("*** 2024-10-02 09:10 before calling  replaceModalFilterRow (grouprule)*** ");
    //replaceModalFilterRow (grouprule);
    console.log("*** 2024-10-02 09:10 after calling  replaceModalFilterRow (grouprule)*** ");
    console.log("*** 2024-10-01 18:44 group rule*** ");
 //   removeElementsByClass();
   appendRowChoices(grouprule);
    stopTheNonsense();
  ...
  */
  ...
  function batchFireStore2(outerArray) {
    ...
    for (i = 0; i < outerArrayLength; i++) {
      ...
      let theDoc = String.fromCodePoint(8456,8448,9884) ; 
